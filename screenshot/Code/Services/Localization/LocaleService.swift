...
enum LocaleService {
    ...
    /// True when `localeCode` resolves every shape in the row unchanged — no style/geometry
    /// override (keyed by shape id) and no translated text (keyed by `textTranslationKey`).
    /// Mirrors `resolveShape`'s guards exactly, so a neutral row renders pixel-identical to
    /// the base locale and export can reuse one render for all such locales. Backgrounds are
    /// not locale-overridable, so shapes are the only inputs that matter.
    static func rowIsLocaleNeutral(row: ScreenshotRow, localeCode: String, localeState: LocaleState) -> Bool {
        ...
        return !row.activeShapes.contains { shape in
            localeState.override(forCode: localeCode, shapeId: shape.id) != nil
                || localeState.overrides[localeCode]?[shape.textTranslationKey] != nil
        }
    }
