    ...
    private var localesEntry: HelpEntry {
        HelpEntry(
            ...
            blocks: [
                ...
                .bullet("A section header shows an amber count when properties inside it are overridden — so a collapsed section still tells you something is there."),
                .bullet("The selected shape's summary sits beside its name at the top of the inspector: the language and how many properties it overrides. Its **reset** button opens a menu listing each overridden property, so you can put one back to the base language or reset them all."),
                .bullet("Each row header shows an amber count of the shapes that language overrides; click it to select them."),
                .bullet("Hovering a shape the current language overrides outlines it in amber instead of the usual accent color."),
    ...
    private var appStoreConnectEntry: HelpEntry {
        HelpEntry(
            ...
            blocks: [
                ...
                .bullet("If every version of the app is live or locked for review, the version step offers **Create Version** — it prefills the next number for each platform the app ships on, and you can edit it before creating."),
                ...
                .bullet("Anything that would block the upload is listed in a banner above the plan, one line per row — every language still missing an App Store locale is named on that one line, not repeated. Where a single change would settle it, the line carries a **Fix** button that makes it: ticking the App Store locales the row already matches, or switching the row to the detected display type."),
                .bullet("If a language has no matching App Store locale, the wizard says so and offers **Create de-DE in App Store Connect** — naming the App Store locale code it would add — right on the row. One click adds it to that version and the row switches to it. Languages the App Store doesn't offer, and versions Apple has locked for editing, still have to be handled on the website; **Refresh App Store data** picks up anything added there."),
    ...
    private var googlePlayEntry: HelpEntry {
        HelpEntry(
            ...
            blocks: [
                ...
                .bullet("Enter your app's **package name** (for example `com.example.myapp`). Google Play has no way to list your apps, so the name is typed rather than picked — the clock button offers the ones you used before."),
                .bullet("**Next** checks the package before going on, so a typo or a service account without access is caught here instead of halfway through an upload."),
                .bullet("Each row has an include switch, an **Upload as** image type (auto-detected from the row's size — the ⓘ button shows the sizes Play accepts), and a checkbox per language."),
                .bullet("Each project language is matched to the Google Play listing language shown beside it. Play keeps one set of screenshots per language, so when two project languages match the same one — **English** and **English (US)** both reach `en-US` — only one can be checked. A language Play doesn't publish in reads **Not on Play** and is skipped."),
                ...
                .bullet("Uploading **replaces** what is already on the listing: for every language and image type you selected, the existing screenshots are deleted and the new ones uploaded. The confirmation says so before anything is sent."),
                ...
                .bullet("Use it to walk through the whole upload flow before you have credentials set up. While demo mode is on, the imported service account is ignored, the package field starts on a sample name, and the package check always passes."),
