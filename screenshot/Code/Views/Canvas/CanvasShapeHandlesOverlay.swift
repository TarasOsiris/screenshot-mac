...
struct CanvasShapeHandlesOverlay: View {
    ...
    /// Latched on the gesture's first tick: `shape` is re-supplied from the document on every body
    /// evaluation, so a mid-drag write would rebase the accumulated translation and jump the shape.
    @State private var resizeBase: CanvasShapeModel?
    ...
    private func rotateGesture(stemLength: CGFloat) -> some Gesture {
        ...
        return DragGesture(minimumDistance: 0, coordinateSpace: .global)
            ...
            .onEnded { _ in
                ...
                var updated = shape
                updated.rotation = CanvasShapeModel.normalizedRotation(shape.rotation + delta)
                onUpdate(updated)
            }
    }
        ...
        .gesture(
            ...
            DragGesture(minimumDistance: 0, coordinateSpace: .global)
                .onChanged { value in
                    ...
                    // A zero-translation tick is the nearest thing SwiftUI gives to "gesture
                    // began", and it still fires after one whose `onEnded` never arrived.
                    if value.translation == .zero || resizeBase == nil { resizeBase = shape }
                    let base = resizeBase ?? shape
                ...
                .onEnded { _ in
                    ...
                    if let resizeState, resizeState.movedFrom(shape) {
                        // Geometry from the latched base; every other field from the live shape.
                        var updated = shape
