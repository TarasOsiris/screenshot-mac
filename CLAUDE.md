...
- **Privacy is load-bearing, not a style preference.** `screnshot-mac-site/app/routes/privacy.tsx` §5 and §6 promise that neither crash reports nor analytics carry projects, screenshots, fonts, or user-written text. So: counts, enum raw values, booleans, durations, and *our* identifiers (bundled template ids, MCP tool names, argument keys) only — never row/project/locale labels, user file names, or text. `options.enableLogs`, `attachScreenshot`, and `attachViewHierarchy` must stay off for the same reason. `beforeSend` scrubs `/Users/<name>` → `/Users/~`; `RowRenderer.reportableRenderLabel` elides the quoted row label that render labels embed. When adding a payload, assume it will be read by someone who must not learn what the user is building.
