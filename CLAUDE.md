...
- `breadcrumb(_ category:_ message:data:level:)` — the trail. A run of identical consecutive crumbs collapses to one (a drag would otherwise evict everything else), so don't rely on repeat counts. The `withUndo`/`withRowUndo` hook in `AppState.swift` already emits one crumb per document mutation, using the action name — new editing operations get a breadcrumb for free.
...
- `setTag` / `setDocumentContext` — persistent scope. The `document` context (counts, device categories, `max_row_pixels`, zoom) refreshes on the debounced save tick in `AppState+Persistence.updateCrashDocumentContext()`; tags are `storage`, `pro`, `mcp`, `crashed_last_run`.
