...
- **One root, threaded end to end.** The flag and the root must come from a single snapshot: `rootURL(isUsingICloud:)` derives the root from the caller's captured flag, and `rebuildIndexFromProjectDirs(at:)` / `preserveUnreadableIndex(at:)` take it explicitly (no default — a default is how the live-global read comes back). Re-reading the global `rootURL` mid-call lets a container resolution land between the guard and the read, so the guard sees `local` while the rebuild scans iCloud.
- The write-back is part of that chain: `loadIndexOrRecover` returns the `root` it read from and callers persist through `AppState.saveIndex(at:)`, never the live-root `saveIndex()`. Pinning only the read moves the same race one step later — a rebuild from local project dirs landing in the iCloud index.
