...
- `ship` — bump build/marketing version, build, upload dSYMs to Sentry, upload to App Store Connect, then register the release in Sentry and associate its commits (`sentry-cli releases new --finalize` + `set-commits --auto`). The release name must be the SDK's own `xyz.tleskiv.screenshot@<MARKETING_VERSION>+<CURRENT_PROJECT_VERSION>` — both platforms share one bundle id, so one release covers both. That association is what powers suspect commits and `Fixes SCREENSHOT-BRO-<id>` auto-resolution.
