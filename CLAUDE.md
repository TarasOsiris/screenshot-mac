...
- `App/` — `AppState` (split across `AppState+*` extensions), app entry, window management, and the cross-cutting controllers (`AppInfo`, `AppNavigationRouter`, `CanvasFocusController`, `CanvasKeyCommandMonitor`, `ContinuousEditCoalescing`, `CustomFontLibrary`, `EditCoalescingCoordinator`, `EditorViewModeController`, `InlineTextEditSession`, `LiveShapeEditSession`, `LocaleMenuCoordinator`, `OnboardingCoach`/`OnboardingCoachController`, `ZoomController`).
...
- `Views/` — `Canvas/` (with `Canvas/Text/` for the inline-text/rich-text subsystem, and `CanvasZoomGestures` for scroll/pinch zoom), `Editor/`, `Export/`, `Help/`, `Inspector/`, `Onboarding/`, `Projects/`, `Settings/`, `Store/`, `Toolbar/` (with `Toolbar/Properties/`, `Toolbar/Popovers/` and `Toolbar/Locale/`), plus the top-level shell views (`ContentView*`, `ActionButton`, `UIMetrics`, `ContextMenuPreviewModifier`, `PlatformViewModifiers`, `DebugProjectManagerView`; `AppRootView` itself lives in `App/`). Export UI is grouped by destination under `Export/AppStoreConnect/`, `Export/GooglePlay/`, `Export/Showcase/`, and `Export/Shared/`.
...
`distinct_id` via `alias()` from `StoreService.start()`. Use `alias`, never `identify`, for that:
...
- `screenshotTests/Views/HelpContentTests.swift` pins what can be checked mechanically: section coverage, distinct icons, the free-tier numbers against `StoreService`, every `DeviceCategory` and `ShowcaseAspectPreset` being named, no duplicate shortcut, and that search still finds the topics behind past support questions. Adding a case to those enums fails Help until it is documented.
