...
- `Export/` — `ExportService`, `ExportImageEncoder`, `ExportFolderService`, plus the flow layer: `ExportFlowModel` (the editor's export state machine — progress, cancellation, temp-folder lifetime, iPad destination routing; behind the `ExportDocument` protocol so it tests without an `AppState`) and `ExportCoordinator`/`ExportFolderBookmark`. Renders templates/showcases to PNG/JPEG via SwiftUI `ImageRenderer` and handles final image encoding.
