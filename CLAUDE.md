...
- `App/` — `AppState` (split across `AppState+*` extensions), app entry, window management, and the cross-cutting controllers (`AppInfo`, `AppNavigationRouter`, `CanvasFocusController`, `ContinuousEditCoalescing`, `EditCoalescingCoordinator`, `OnboardingCoach`/`OnboardingCoachController`).
